...
class PortHandler(object):
    ...
    async def readPort_async(self, length):
        if (sys.version_info > (3, 0)):
            return await self.ser.read(length)
        else:
            return [ord(ch) for ch in await self.ser.read(length)]

    def writePort(self, packet):
        return self.ser.write(packet)

    async def writePort_async(self, packet):
        return await self.ser.write(packet)
    ...
    def setupPort(self, cflag_baud):
        ...
        self.ser = aioserial.AioSerial(port=self.port_name, baudrate=self.baudrate,
